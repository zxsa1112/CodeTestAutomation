...
public class GWTTests {
    ...
    private Collection<DynamicTest> createGWTTestsForStockTrading() { // 주식 거래 시스템에 대한 GWT 테스트를 생성하는 메소드
        ...
        tests.add(DynamicTest.dynamicTest("삼성 주식을 10주 구매할 때 성공해야 한다", () -> {
            ...
            double initialBalance = stockTrading.getAccountBalance(); // 초기 잔액 조회

            // 삼성 주식 10주 구매 시도
            boolean result = stockTrading.buyStock("삼성", 10); // 인자 수정

            // 주식 구매 성공 여부 및 잔액 검증
            Assertions.assertTrue(result, "주식 구매가 성공해야 합니다.");
            double currentBalance = stockTrading.getAccountBalance(); // 현재 잔액 조회
        ...
        tests.add(DynamicTest.dynamicTest("현대 주식을 2주 판매할 때 성공해야 한다", () -> {
            ...
            stockTrading.buyStock("현대", 5); // 인자 수정
            double initialBalance = stockTrading.getAccountBalance(); // 초기 잔액 조회

            // 현대 주식 2주 판매 시도
            boolean result = stockTrading.sellStock("현대", 2); // 인자 수정
            ...
            // 주식 수 검증
            int remainingStocks = stockTrading.getStockCount("현대"); // 인자 수정
